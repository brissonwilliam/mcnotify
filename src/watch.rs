...
fn get_log_event(logline :&str) -> Option<McLogEvent> {
    ...
    if let Some(captures) = disconnect_regex.captures(logline) {
        ...
        let msg = format!("{} has disconected from the server. Sadge", username.as_str());
...
"#, event.msg, event.color);
    ...
    fn on_modify(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        while n > 0 {
            ...
            let log_event = match get_log_event(str.as_str()) {
                Some(event) => event,
                None => continue,
            };
        ...
        loop {
            ...
            for event in events {
                ...
                if event.mask.contains(EventMask::DELETE) || event.mask.contains(EventMask::DELETE_SELF) || event.mask.contains(EventMask::MOVE_SELF) {
                    ...
                    println!("received {:?} event, stopping watch descriptor #{}", event.mask, event.wd.get_watch_descriptor_id());
                    ...
                    println!("received {:?} event on watched path, adding new watch", event.mask);
                    inotify.watches()
                        .add(self.path.as_str(), file_flags)
                        .expect("Failed to add file watch");
